...
- FR2.1 `itx session new` creates a session node under the current project (creating
  the project node under the root on first use) and returns its id. Supports bulk
  creation from a JSON file (`--from manifest.json`) describing tasks + dependencies.
...
- FR2.5 `itx session show <id>` reports full session state — the session node plus its
  task subtree (the **manifest** view) — including per-task status and terminal-window
  liveness.
...
- FR2.7 State layout:
  ...
  - `~/.config/itx/config.yml` — user configuration (default harness, storage
    backend, etc.).
- FR2.8 Concurrent writers (kernel loop + N task windows) MUST NOT corrupt or lose
...
- FR3.2 One terminal **window per task** (named
  ...
  by default; optional `max_parallel` cap in config).
...
- FR3.5 Status updates are belt & suspenders:
  ...
  - The kernel loop polls and reconciles: dead window while `running` ⇒ `failed`;
    `done` task ⇒ tear down window, unblock and spawn dependents; `failed` task ⇒
    dependents `blocked`.
- FR3.6 `itx session stop <id>` pauses execution (kills kernel loop and task windows;
...
- NFR1 Deterministic kernel: given the same DAG, the kernel loop makes the same
  spawn/reconcile decisions. LLM calls only ever happen through the LLM adapter,
  never inside scheduling/reconciliation logic.
- NFR2 Crash safety: kernel loop restart recovers from the DAG file alone.
